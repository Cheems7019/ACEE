...
def run_for_x_dim(x_dim: int, args) -> None:
    data_dir = os.path.join(args.data_root, f"M4_xdim_{x_dim}")
    if not os.path.isdir(data_dir):
        warnings.warn(f"Data dir not found: {data_dir}")
        return

    os.makedirs(args.results_root, exist_ok=True)
    results_file = os.path.join(args.results_root, f"M4_time_xdim_{x_dim}.csv")
    ...
    for seed in range(args.n_seeds):
        print(f"\n{'='*70}")
        print(f"X_DIM {x_dim} | SEED {seed}/{args.n_seeds - 1}")
        print(f"{'='*70}")
        ...
        for n_size in args.n_sizes:
            ...
            mc_samples = args.mc_total // n_size
            if mc_samples < 1:
                mc_samples = 1
            if args.mc_total % n_size != 0:
                print(
                    f"Warning: mc_total {args.mc_total} not divisible by n_size {n_size}. "
            ...
            print(
                f"n={n_size} | total={total_time:.2f}s | "
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Computation scalability for M4 (vary x_dim)."
    )
    ...
    parser.add_argument(
        "--x_dims",
        type=parse_int_list,
        default=parse_int_list("10,20,50,100"),
    )
    parser.add_argument(
        "--n_sizes",
        type=parse_int_list,
    ...
    parser.add_argument(
        ...
        type=int,
        default=100000,
        help="Total n_size * mc_samples budget (mc_samples = mc_total / n_size).",
    )
