...
def run_for_n_size(n_size: int, data_dir: str, args) -> None:
    os.makedirs(args.results_root, exist_ok=True)
    results_file = os.path.join(args.results_root, f"M4_time_n{n_size}.csv")
    ...
    for seed in range(args.n_seeds):
        print(f"\n{'='*70}")
        print(f"N_SIZE {n_size} | SEED {seed}/{args.n_seeds - 1}")
        print(f"{'='*70}")
        ...
        print(
            f"x_dim={x_dim_used} | total={total_time:.2f}s | "
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Computation scalability for M4 (vary n_size)."
    )
    ...
    for n_size in args.n_sizes:
        run_for_n_size(n_size, data_dir, args)
