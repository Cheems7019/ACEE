#!/usr/bin/env python3
"""
summary_nonlin_simpson_um.py - Summarize ATE error from nonlin_simpson_UM.py.
...
SCENARIOS = {
    "additive": {
        "results_dir": "nonlin_simpson_um",
        "data_dir": "nonlin_simpson_um",
    },
    "nonadditive": {
        "results_dir": "nonlin_simpson_nonadditive_um",
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Summarize nonlin_simpson_UM ATE absolute error across additive and nonadditive scenarios."
    )
    parser.add_argument(
        "--scenarios",
    ...
    output_path = (
        ...
        if args.output
        else results_root / "summary_nonlin_simpson_um_ate_abs_error.csv"
    )
    scenario_frames = []
