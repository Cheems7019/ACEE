...
SCENARIOS = {
    "additive": "nonlin_simpson_um",
    "nonadditive": "nonlin_simpson_nonadditive_um",
}
...
def run_for_data_dir(
    ...
    if not os.path.isdir(data_dir):
        warnings.warn(f"Data dir not found, skipping {label}: {data_dir}")
        return

    n_sizes = sorted(set(args.n_sizes))
    if n_sizes != [500]:
        warnings.warn(
            f"{label}: factor extraction supports only n_500. Using [500] instead of {n_sizes}."
        )
        n_sizes = [500]
    ...
    if not seeds:
        warnings.warn(f"No seeds found for {label} in {data_dir}")
        return

    print("\n" + "=" * 70)
    print(f"NONLIN_SIMPSON_UM FACTOR EXTRACTION: {label}")
    print("=" * 70)
        ...
        for n_size in n_sizes:
            ...
            for k in args.pca_components:
                ...
                try:
                    ...
                    pca_file = os.path.join(data_subdir, f"seed_{seed}_pca_k{k}_z.csv")
            ...
            if args.pca_adaptive:
                try:
                    ...
                    print(f"Saved adaptive PCA factors (k={k_adapt}) to {pca_file}")
                except Exception as exc:
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Compute latent factors for nonlin_simpson_UM (X-only, n_500)."
    )
    ...
    if args.data_dir:
        run_for_data_dir("custom", args.data_dir, args)
        return

    for scenario in args.scenarios:
        dataset_dir = os.path.join(data_root, SCENARIOS[scenario])
        run_for_data_dir(scenario, dataset_dir, args)
