...
DATASETS = {
    "additive": {
        "data_dir": "nonlin_simpson_um",
        "result_dir": "nonlin_simpson_um",
        "intervention_idx": 0,
        "outcome_idx": 2,
        "x0_low": -1.0,
        "x0_high": 1.0,
    ...
    "nonadditive": {
        "data_dir": "nonlin_simpson_nonadditive_um",
        "result_dir": "nonlin_simpson_nonadditive_um",
        "intervention_idx": 0,
        "outcome_idx": 2,
        "x0_low": -1.0,
        "x0_high": 1.0,
...
def estimate_ate_single(
    ...
    low_norm = scale_column_value(scaler, x0_low, intervention_idx, d_in)
    high_norm = scale_column_value(scaler, x0_high, intervention_idx, d_in)
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="nonlin_simpson_UM with conditional diffusion model using Z factors."
    )
    ...
    if n_sizes != [500]:
        warnings.warn(
            f"nonlin_simpson_UM supports only n_500. Using [500] instead of {n_sizes}."
        )
        n_sizes = [500]

    print("\n" + "=" * 70)
    print("NONLIN_SIMPSON_UM EXPERIMENT")
    print("=" * 70)
    ...
    base_adj = build_nonlin_simpson_adj()
